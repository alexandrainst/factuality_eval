...
install: ## Install dependencies
	@echo "Installing the 'factuality_eval' project..."
	...
	@echo "Installed the 'factuality_eval' project! You can now activate your virtual environment with 'source .venv/bin/activate'."
...
add-repo-to-git:
	...
	@if [ "$(shell git remote)" = "" ]; then \
		git remote add origin git@github.com:alexandrainst/factuality_eval.git; \
	fi
...
publish-docs:  ## Publish documentation to GitHub Pages
	...
	@echo "Updated documentation website: https://alexandrainst.github.io/factuality_eval"

test:  ## Run tests
	@uv run pytest && uv run readme-cov

docker:  ## Build Docker image and run container
	@docker build -t factuality_eval .
	@docker run -it --rm factuality_eval
